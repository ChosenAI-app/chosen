...
const STEPS = [
  { label: "Reading parcel records", detail: "ATTOM property data + APN lookup...", pct: 8 },
  { label: "Checking fire review triggers", detail: "West of 280, sprinkler requirements...", pct: 20 },
  { label: "Mapping Accela permit sequence", detail: "Building, electrical, plumbing, mechanical...", pct: 35 },
  { label: "Calculating PAUSD and impact fees", detail: "School fees, development charges...", pct: 50 },
  { label: "Estimating subcontractor costs", detail: "Bay Area 2026 market rates...", pct: 63 },
  { label: "Running code compliance check", detail: "CBC, Title 24, CalGreen...", pct: 76 },
  { label: "Identifying plan check risks", detail: "Common correction letter items...", pct: 88 },
  { label: "Generating project brief", detail: "Writing contractor scope summary...", pct: 96 },
]

export function ContractorScopePoller({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(async () => {
      ...
      try {
        const res = await fetch(`/api/contractor/project-status?id=${projectId}`)
        if (!res.ok) return
  ...
  return (
    <div className="relative flex min-h-[400px] w-full flex-col items-center justify-center overflow-hidden rounded-lg border border-border bg-[#0a0d14]">
      <div
      ...
      <div className="absolute bottom-0 left-0 right-0 border-t border-border/50 bg-black/40 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="font-mono text-[10px] text-green-400/80">chosen-ai · contractor-engine · {elapsed}s elapsed</p>
          </div>
