...
export function ProjectChatWindow({ projectId }: { projectId: string }) {
    ...
    <div className="flex h-80 flex-col rounded-lg border border-border bg-card">
        ...
        <div className="flex flex-col gap-2">
          {messages.length === 0 && (
            <p className="py-8 text-center text-xs text-muted-foreground">
              Ask me anything about your project, permits, costs, or timeline.
            </p>
          )}
          {messages.map((m) => {
            const text = m.parts
      ...
      <div className="flex gap-2 border-t border-border p-2">
        <textarea
          ...
          rows={1}
          placeholder="Ask about permits, costs, timeline..."
