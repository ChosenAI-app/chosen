...
export async function generateFormFills(
  ...
  const zipCode = project.zip_code ?? "94301"
  const ownerName = profile?.full_name ?? ""
  const ownerPhone = profile?.phone ?? null
  const lotSqft = project.lot_size_sqft
  const yearBuilt = project.year_built
  const apn = project.regrid_parcel_id
  const hasSprinklers = project.fire_sprinklers_exist ?? false
  const separatelyMetered = project.separately_metered ?? false

  // Cost estimate from AI scope
  ...
  const buildingFilled: Record<string, string | boolean | null> = {
    ...
    parcel_number: apn ?? null,
    zoning: project.zoning_description ?? project.zoning ?? "R-1",
    description_of_work: buildDescription(project),
    estimated_valuation: `$${estimatedCost.toLocaleString()}`,
    new_construction_sqft: project.adu_sqft ? String(project.adu_sqft) : null,
      ...
      ? [
          {
            ...
            reason: "Look up at sccassessor.org or your property tax bill",
            input_type: "text" as const,
          },
        ]
      : []),
  ...
  for (const form of forms) {
    const needsReview = (form.flagged_fields as FlaggedField[]).length > 0
    ...
    if (error) {
      console.error("[generateFormFills] error for", form.form_key, error)
      return { error: `Failed to save ${form.form_name}: ${error.message}` }
    }
  }

  console.log("[generateFormFills] inserted", forms.length, "forms")
...
export async function updateFlaggedField(
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      status: updatedFlagged.length === 0 ? "approved" : "needs_review",
      updated_at: new Date().toISOString(),
    })
    .eq("id", formFillId)
